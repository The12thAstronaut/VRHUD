...
public class EyeTrackingExample : MonoBehaviour
{
    ...
    public class VarjoTestET : MonoBehaviour
    {
        void Update()
        {
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                {
                    Debug.Log("Invoke calibration fast mode");
                    Varjo.XR.VarjoEyeTracking.RequestGazeCalibration(Varjo.XR.VarjoEyeTracking.GazeCalibrationMode.Fast);
                }
                else
                {
                    Debug.Log("Invoke calibration legacy mode");
                    Varjo.XR.VarjoEyeTracking.RequestGazeCalibration(Varjo.XR.VarjoEyeTracking.GazeCalibrationMode.Legacy);
                }
            }
        }
    }
    ...
    void Update()
    {
        ...
        if (TriggerActive_R && !showGestureMode)
        {
            ...
            if (TriggerActive_L && timeStart <= 1)
            {           //checking to see if the left trigger is hit in 2 seconds
        ...
        if (TriggerActive_L && !hideGestureMode)
        {
            ...
            if (TriggerActive_R && timeStart <= 1)
            {           //checking to see if the left trigger is hit in 2 seconds
                menu.SetActive(true);
        ...
        if (TriggerActive_U && !scrollUpGestureMode)
        {
            ...
            if (TriggerActive_D && timeStart <= 1)
            {           //checking to see if the left trigger is hit in 2 seconds
        ...
        if (TriggerActive_D && !scrollDownGestureMode)
        {
            ...
            if (TriggerActive_U && timeStart <= 1)
            {           //checking to see if the left trigger is hit in 2 seconds
        ...
        if (Physics.SphereCast(rayOrigin, gazeRadius, direction, out hit))
        {
            ...
            varjoFocus = hit.collider.gameObject;
            //Print the name of the game object our eyes are looking at
            print("The object is:" + varjoFocus);


            //Change the color of the gameobject we are looking it if it is a trigger plane
            if (varjoFocus.name.StartsWith("Trigger"))
            {
                varjoFocus.GetComponent<Renderer>().material.color = HighlightColor;

                //trigger R
                if (varjoFocus.name == "Trigger_R")
                {
            ...
            //If the current focus is different than the previous, change the previous color back

            if (varjoFocus.name != "Trigger_R")
            {
                Trigger_R.GetComponent<Renderer>().material.color = OriginalColor;
            }

            if (varjoFocus.name != "Trigger_L")
            {
                Trigger_L.GetComponent<Renderer>().material.color = OriginalColor;
            }

            if (varjoFocus.name != "Trigger_U")
            {
                Trigger_U.GetComponent<Renderer>().material.color = OriginalColor;
            }
            if (varjoFocus.name != "Trigger_D")
            {
                Trigger_D.GetComponent<Renderer>().material.color = OriginalColor;
            }
